...
public class EngineBar extends View {
	...
	public static final int BR_WIDTH = 30;
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawRect(new Rect(0, getControlBottom(), getWidth(),
				getControlBottom() + CTRL_HEIGHT), controlPaint);
	}

	@Override
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		setMeasuredDimension(BR_WIDTH, getDefaultSize(
	...
	public void setValue(float value) {
		if (Math.abs(value) > getAbsoluteMax()) {
			this.value = getAbsoluteMax() * Math.signum(value);
		} else {
			this.value = value;
		}
	}
